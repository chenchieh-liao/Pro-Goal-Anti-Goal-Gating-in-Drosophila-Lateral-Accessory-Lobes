...
plt.plot(x, mean30, label='30˚, no Exc. core output', linestyle='--', color='orange',linewidth=8)
plt.plot(x, mean90, label='90˚, no Exc. core output', linestyle='--', color='green',linewidth=8)
...
ax.spines['bottom'].set_linewidth(4)  # 加粗 X 軸線條
for spine in ax.spines.values():
    spine.set_linewidth(4)
ax.tick_params(axis='both', which='both', width=4,length=10)  # 加粗刻度線

plt.tight_layout(rect=[0.03, 0.03, 0.97, 0.97])  # 增加邊界空間
plt.savefig('fig2D_v3.svg')
