...
def plot_for_group(folder,folder2, output_filename, title):
    output_files = [
    ...
    output_files2 = [
        os.path.join(folder2, f'dna02_180_0.01_DNa02.txt'),
        os.path.join(folder2, f'dna02_180_0.01_LAL018.txt'),
        os.path.join(folder2, f'dna02_180_0.01_LAL040.txt'),
        os.path.join(folder2, f'dna02_180_0.01_LAL121.txt'),
    ]
    # Create subplots
    fig, axes = plt.subplots(nrows=1, ncols=4, figsize=(18, 5), sharey=False)
    axes = axes.flatten()

    for i, (ax, file,file2) in enumerate(zip(axes, output_files,output_files2)):
        # Read data from file
        x, y1, y2 = [], [], []
        ...
        with open(file2, 'r') as f2:
            for line in f2:
                ...
                y12.append(float(data[1]))
                y22.append(float(data[2]))

        # Plot data
        ax.plot(x, y1, label='left', color='blue', linewidth=line_width)
        ax.plot(x, y2, label='right', color='red', linewidth=line_width)
        ax.plot(x2, y12, label='left', color='blue', linestyle='--', linewidth=line_width, alpha=0.4)
        ax.plot(x2, y22, label='right', color='red', linestyle='--', linewidth=line_width, alpha=0.4)
...
plot_for_group(group1_folders[1], group1_folders[0], 'fig3d_no121_180.svg', '– DI2 output')
plot_for_group(group1_folders[2], group1_folders[0], 'fig3d_noEC_180.svg', '– EC output')
