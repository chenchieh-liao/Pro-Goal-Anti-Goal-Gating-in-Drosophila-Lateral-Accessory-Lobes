...
# 繪製 ax7
ax7.plot(trace_lal046[0].t/ms, trace_lal046[0].r, color='red', label='right lal046', linewidth=line_width)
ax7.plot(trace_lal046[1].t/ms, trace_lal046[1].r, color='blue', label='left lal046', linewidth=line_width)
ax7.title.set_text('LAL046')
ax7.title.set_size(24)
ax7.tick_params(axis='both', labelsize=14)

# 繪製 ax8
ax8.plot(trace_lal121[0].t/ms, trace_lal121[0].r, color='red', label='right lal121', linewidth=line_width)
ax8.plot(trace_lal121[1].t/ms, trace_lal121[1].r, color='blue', label='left lal121', linewidth=line_width)
...
for ax in [ax2, ax3, ax4, ax5, ax7, ax8, ax9, ax10]:
    ...
    ax.tick_params(left=False)  # 隱藏 y 軸刻度
    ax.set_yticklabels([])  # 移除 y 軸標籤
    ax.tick_params(labelbottom=False)  # 隱藏 x 軸標籤，但保留刻度
...
# 保存圖片
plt.savefig(f'fig2b_{seed_}.png')
