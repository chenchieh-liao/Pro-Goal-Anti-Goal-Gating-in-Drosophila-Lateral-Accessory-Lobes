...
for data in range(1, 51):
    ...
    #file120ctr = pd.read_csv(f'/home/chieh1102/FPLmodel/open-loop_v5/fig2b/data_ctr/dna02_120_0.01_{data}.txt', header=None, delimiter=' ')
...
plt.plot(x, mean10, label='10˚, no Exc. core output', color='blue',linewidth=5)
plt.plot(x, mean30, label='30˚, no Exc. core output', color='orange',linewidth=5)
plt.plot(x, mean90, label='90˚, no Exc. core output', color='green',linewidth=5)
# plt.plot(x, mean120, label='120˚, no 010&014', color='red')
plt.plot(x, mean10ctr, label='10˚, full model', linestyle='--', color='blue',linewidth=5)
plt.plot(x, mean30ctr, label='30˚, full model', linestyle='--', color='orange',linewidth=5)
plt.plot(x, mean90ctr, label='90˚, full model', linestyle='--', color='green',linewidth=5)
# plt.plot(x, mean120ctr, label='120˚, full model', linestyle='--', color='red')
#plt.legend(fontsize = 18)

plt.xlabel('time (ms)', fontsize=24)
plt.ylabel('mean firingrate difference of DNa02', fontsize=24)
...
ax.spines['left'].set_linewidth(1.0)  # 加粗 Y 軸線條
ax.spines['bottom'].set_linewidth(1.0)  # 加粗 X 軸線條

plt.tight_layout(rect=[0.01, 0.01, 0.99, 0.99])  # 增加邊界空間
plt.savefig('fig2F.png')
