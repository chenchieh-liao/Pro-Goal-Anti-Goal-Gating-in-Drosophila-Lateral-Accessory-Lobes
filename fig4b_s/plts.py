...
data_anti_dir = os.path.join(base_dir, 'data_ori')
...
for i, (ax, file1, file2) in enumerate(zip(axes, output_files_group1, output_files_group2)):
    ...
    # 繪圖
    ax.plot(x1, y1_1, color='blue', linewidth=line_width, label='L Anti-goal')
    ax.plot(x1, y1_2, color='red', linewidth=line_width, label='R Anti-goal')
    ax.plot(x2, y2_1, color='blue', linewidth=line_width, alpha=0.3, label='L Pro-goal')
    ax.plot(x2, y2_2, color='red', linewidth=line_width, alpha=0.3, label='R Pro-goal')
...
plt.savefig('fig4C_s.png',dpi = 90)
plt.show()
