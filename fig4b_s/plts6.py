...
# 定義資料夾
data_ori_dir = os.path.join(base_dir, 'data_xantiTopro')
...
for i, (ax, file1, file2) in enumerate(zip(axes, output_files_group1, output_files_group2)):
    ...
    # 繪圖
    ax.plot(x1, y1_1, color='blue', linewidth=line_width, label='L – anti → pro')
    ax.plot(x1, y1_2, color='red', linewidth=line_width, label='R – anti → pro')
...
plt.savefig('fig4H_s.png',dpi= 90)
plt.show()
