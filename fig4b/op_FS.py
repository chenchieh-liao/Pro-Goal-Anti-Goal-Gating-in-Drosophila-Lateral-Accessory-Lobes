...
dr/dt = -r/tau + (1/(1+exp(-s1 * (((I_pfl3 + I_pfl2 + I_lal + I))-s2))))/tau : 1
dI/dt = -(I - I_mean)/tau_I + sigma*sqrt(2/tau_I)*xi*abs(I_mean) : 1
tau : second
tau_I : second
I_mean : 1
s1: 1
...
LAL153[1].I_mean = int(sys.argv[3])/2
LAL046[1].I_mean = int(sys.argv[3])/2
LAL017[0].I_mean = int(sys.argv[3])*0.2/2

LAL153[0].I_mean = int(sys.argv[3])/2
LAL046[0].I_mean = int(sys.argv[3])/2
LAL017[1].I_mean = int(sys.argv[3])*0.2
...
base_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data_Front')

suffixes = [
    'DNa01', 'DNa02', 'DNa03', 'DNa04',
    'LAL010', 'LAL014', 'LAL018', 'LAL040', 'LAL046',
    'LAL121', 'LAL073', 'LAL122', 'LAL126', 'LAL153', 'LAL017'
]

# 直接在「這個檔案夾」底下找檔案
output_files = [
    os.path.join(base_dir, f'dna02_{inp_g - inp_h}_{sys.argv[2]}_{s}.txt')
    for s in suffixes
]
