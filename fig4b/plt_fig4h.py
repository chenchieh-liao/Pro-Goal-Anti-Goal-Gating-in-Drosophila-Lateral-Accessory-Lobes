...
data_anti_dir = os.path.join(base_dir, 'data_xantiTopro')
...
for i, (ax, file1, file2) in enumerate(zip(axes, output_files_group1, output_files_group2)):
    ...
    ax.plot(x2, y2_1, label='— anti→pro L', color='blue', linewidth=line_width)
    ax.plot(x2, y2_2, label='— anti→pro R', color='red', linewidth=line_width)
