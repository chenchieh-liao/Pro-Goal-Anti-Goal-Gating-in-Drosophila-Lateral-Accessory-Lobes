...
# 定義資料夾
data_ori_dir = os.path.join(base_dir, 'data_ori')
...
for i, (ax, file1, file2) in enumerate(zip(axes, output_files_group1, output_files_group2)):
    ...
    # 繪圖
    ax.plot(x1, y1_1, label='Pro-goal L', color='blue', alpha=0.3, linewidth=line_width)
    ax.plot(x1, y1_2, label='Pro-goal R', color='red', alpha=0.3, linewidth=line_width)
    ax.plot(x2, y2_1, label='Anti-goal L', color='blue', linewidth=line_width)
    ax.plot(x2, y2_2, label='Anti-goal R', color='red', linewidth=line_width)
