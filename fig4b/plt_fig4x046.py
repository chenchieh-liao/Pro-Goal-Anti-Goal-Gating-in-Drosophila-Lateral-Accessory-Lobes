...
data_anti_dir = os.path.join(base_dir, 'data_x046')
...
for i, (ax, file1, file2) in enumerate(zip(axes, output_files_group1, output_files_group2)):
    ...
    # 繪圖
    ax.plot(x1, y1_1, label='Full model L', color='blue', alpha=0.3, linewidth=line_width)
    ax.plot(x1, y1_2, label='Full model R', color='red', alpha=0.3, linewidth=line_width)
    ax.plot(x3, y3_1, label='LAL046 lesion L', color='blue', linestyle='--', linewidth=line_width)
    ax.plot(x3, y3_2, label='LAL046 lesion R', color='red', linestyle='--', linewidth=line_width)
    ...
    if i % 4 == 0:
        ...
        ax.tick_params(axis='y', labelsize=16, width=4, length=10)
    else:
        ax.set_yticks([])
        ax.tick_params(labelleft=False, width=4, length=10)
        ax.spines['left'].set_visible(False)
    xticks = np.arange(0, 801, 500) 
    # 只顯示最底排子圖的 x 軸
    if i == 4:
        ax.set_xlabel('Time (ms)', fontsize=22)
        ax.tick_params(axis='x', labelsize=16, width=4, length=10)
    else:
        ax.set_xticks(xticks)
        ax.tick_params(labelbottom=False, width=4, length=10)

# 特定子圖額外標籤
axes[0].set_ylabel('Firing rate', fontsize=22)
axes[4].set_ylabel('Firing rate', fontsize=22)
axes[4].set_xlabel('Time (ms)', fontsize=22)
