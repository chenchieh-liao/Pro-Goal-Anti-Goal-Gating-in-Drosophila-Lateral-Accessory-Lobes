...
# 資料夾
group1_dir = os.path.join(base_dir, 'data_nogoal')
group2_dir = os.path.join(base_dir, 'data_nogoal_x122-153')
...
# 拼出完整路徑
output_files_group1 = [os.path.join(group1_dir, f) for f in filenames]
output_files_group2 = [os.path.join(group2_dir, f) for f in filenames]
...
for i, (ax, file1, file2) in enumerate(zip(axes, output_files_group1, output_files_group2)):
    ...
    # 繪圖
    ax.plot(x1, y1_1, label='without goal input L', color='blue', alpha=0.3, linewidth=line_width)
    ax.plot(x1, y1_2, label='without goal input R', color='red', alpha=0.3, linewidth=line_width)
    ax.plot(x2, y2_1, label='— without goal input & 122→153 L', color='blue', linewidth=line_width)
    ax.plot(x2, y2_2, label='— without goal input & 122→153 R', color='red', linewidth=line_width)
