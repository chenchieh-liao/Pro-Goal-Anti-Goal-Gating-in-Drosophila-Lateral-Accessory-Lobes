...
inhead.Iin[headn2] = hIn2
inhead.Iin[headn1] = hIn1
ingoal.Iin[goaln2] = gIn2
ingoal.Iin[goaln1] = gIn1
...
dna2_R = np.mean(trace_DNa02[0].r[0:8000])
dna2_L = np.mean(trace_DNa02[1].r[0:8000])
...
for i in range(12):
    PFL3_R = PFL3_R + np.mean(trace[2*i].r[0:8000])
    PFL3_L = PFL3_L + np.mean(trace[2*i+1].r[0:8000])

for i in range(12):
    PFL2 = PFL2 + np.mean(trace_pfl2[i].r[0:8000])


LALinput = sys.argv[3]
f = open('PFL3+PFL2_firing_rate_change_goal.txt', 'a+')
print(f'{initialgoal},{PFL3_R},{PFL3_L},{PFL2}',file = f)
f.close()
